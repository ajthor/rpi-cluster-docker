docker:
  images:
    ...
    - base:
      - image: rpi-cluster/alpine

    - alpine:
      - image: rpi-cluster/alpine
      - ext_image: resin/raspberrypi3-alpine
      - tag: 3.5.2
      # You only need to specify this if the `use_external_images` is `True`.
      - download_url: https://nl.alpinelinux.org/alpine/v3.5/releases/armhf/alpine-minirootfs-3.5.2-armhf.tar.gz
      - sha256:
    ...
    - node:
      - image: rpi-cluster/node
      - ext_image: resin/raspberrypi3-alpine-node
      - tag: 7.7.3
    ...
    - golang:
      - image: rpi-cluster/golang
      - ext_image: resin/raspberrypi3-alpine-golang
      - tag: 1.8
    ...
    - registry:
      - image: rpi-cluster/registry
      - ext_image: vdavy/registry-arm
      - tag: 2.6.1
